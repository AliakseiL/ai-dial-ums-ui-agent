...
class HttpMCPClient:
    ...
    async def get_tools(self) -> list[dict[str, Any]]:
        ...
        logger.info("Retrieved tools from MCP server", extra={"tools": dial_tools})
        return dial_tools
    ...
    async def call_tool(self, tool_name: str, tool_args: dict[str, Any]) -> Any:
        ...
        if not self.session:
            raise RuntimeError("MCP client is not connected to MCP server")
        logger.info("Calling tool on MCP server", extra={"tool_name": tool_name, "tool_args": tool_args, "server_url": self.server_url})
